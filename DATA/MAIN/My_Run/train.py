...
class ChestXray(Dataset):
    ...
    def __len__(self):
        return len(self.data) #Tital number of samples in the dataset

    def __getitem__(self, idx): #Retrieves the image and caption at the specified index
...
model_path="/content/drive/MyDrive/COLAB/clip_epoch_80.pt"  # Path to the saved model checkpoint

model, preprocess = clip.load("ViT-B/32", device=device, jit=False)
ft_model = CLIPFineTune(model).to(device) 

if model_path:
    state_dict = torch.load(model_path, map_location=device) # INCLUDE IF STATEMENT
